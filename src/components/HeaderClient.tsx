...
export default function HeaderClient() {
  ...
  if (isParentSection) {
    return (
      <header style={{ background: "#064E3B", borderBottom: "1px solid #065f46" }}>
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <Link href="/rodice/dashboard" className="flex items-center gap-3 flex-1 min-w-0">
            <div
              className="w-9 h-9 rounded-xl flex items-center justify-center text-lg select-none shrink-0"
              style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.25)" }}
            >
              👨‍👩‍👧
            </div>
            <div className="min-w-0">
              <p className="text-base font-bold leading-tight text-white">Rodičovský portál</p>
              <p className="text-xs leading-tight" style={{ color: "#6ee7b7" }}>MateMax · přehled dítěte</p>
            </div>
          </Link>
          {authReady && userEmail && (
            <Link
              href="/rodice/nastaveni"
              className="text-xs font-semibold px-2.5 py-1 rounded-lg transition-colors hidden sm:flex items-center gap-1"
